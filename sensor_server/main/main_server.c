 ...
 *
 * Changes: same as main.c of this repository (periodic publication of an
 * application sequence number), with a 500 ms transmission period instead
 * of 100 ms. Used for the relaxed-traffic server node.
 */
...
NET_BUF_SIMPLE_DEFINE_STATIC(sensor_data_0, 5);
NET_BUF_SIMPLE_DEFINE_STATIC(sensor_data_1, 5);
...
static esp_ble_mesh_sensor_state_t sensor_states[2] = {
    ...
    [0] = {
        ...
        .sensor_data = {
            ...
            .length = 4, /* 4 means a 5-byte length (the field is zero-based) */
...
static void mesh_stress_test_task(void *pvParameters)
{
    esp_ble_mesh_model_t *sensor_model = (esp_ble_mesh_model_t *)pvParameters;
    static uint32_t stress_seq_num = 0;
    
    /* Message context configuration to force hardware routing */
    esp_ble_mesh_msg_ctx_t ctx = {
        .net_idx = 0x0000,
        .app_idx = 0x0000,      /* Default AppKey index from example */
        .addr = 0x0001,         /* Target address set to Provisioner/Client (always 0x0001) */
        .send_ttl = 7,
        .send_rel = false,      /* Unacknowledged message to avoid network flooding */
    };

    ESP_LOGI(TAG, "Stress Test Task started. Entering periodic publication loop.");

    while (1) {
        /* Hold execution until the provisioning process is complete */
        /*if (!is_provisioned) {
            vTaskDelay(pdMS_TO_TICKS(1000));
            continue;
        }*/

        /* Reset the static buffer memory before writing new data */
        net_buf_simple_reset(&sensor_data_0);

        /* Inject dummy indoor temperature (1 byte) */
        net_buf_simple_add_u8(&sensor_data_0, indoor_temp);

        /* Inject the dynamic Sequence Number (4 bytes Little-Endian) */
        net_buf_simple_add_u8(&sensor_data_0, (uint8_t)(stress_seq_num & 0xFF));
        net_buf_simple_add_u8(&sensor_data_0, (uint8_t)((stress_seq_num >> 8) & 0xFF));
        net_buf_simple_add_u8(&sensor_data_0, (uint8_t)((stress_seq_num >> 16) & 0xFF));
        net_buf_simple_add_u8(&sensor_data_0, (uint8_t)((stress_seq_num >> 24) & 0xFF));

        /* Send the Sensor Status message bypassing the Publish Address state restrictions */
        esp_err_t err = esp_ble_mesh_server_model_send_msg(sensor_model, &ctx, 
                            ESP_BLE_MESH_MODEL_OP_SENSOR_STATUS, sensor_data_0.len, sensor_data_0.data);
        
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "Mesh message transmission failed: %d", err);
        } else {
            ESP_LOGI(TAG, "Successfully sent packet stress_seq_num: %" PRIu32, stress_seq_num);
        }

        stress_seq_num++;
        
        /* 500ms sampling period to stress test the transmission medium */
...
static void prov_complete(uint16_t net_idx, uint16_t addr, uint8_t flags, uint32_t iv_index)
{
    ...
    is_provisioned = true;
    // Initialize the application sequence number
    /*static uint32_t app_seq_num = 0;

    // Fill the buffer of sensor 0 (Indoor)
    net_buf_simple_add_u8(&sensor_data_0, indoor_temp);
    net_buf_simple_add_u8(&sensor_data_0, (uint8_t)(app_seq_num & 0xFF));
    net_buf_simple_add_u8(&sensor_data_0, (uint8_t)((app_seq_num >> 8) & 0xFF));
    net_buf_simple_add_u8(&sensor_data_0, (uint8_t)((app_seq_num >> 16) & 0xFF));
    net_buf_simple_add_u8(&sensor_data_0, (uint8_t)((app_seq_num >> 24) & 0xFF));

    // Fill the buffer of sensor 1 (Outdoor)
    net_buf_simple_add_u8(&sensor_data_1, outdoor_temp);
    net_buf_simple_add_u8(&sensor_data_1, (uint8_t)(app_seq_num & 0xFF));
    net_buf_simple_add_u8(&sensor_data_1, (uint8_t)((app_seq_num >> 8) & 0xFF));
    net_buf_simple_add_u8(&sensor_data_1, (uint8_t)((app_seq_num >> 16) & 0xFF));
    net_buf_simple_add_u8(&sensor_data_1, (uint8_t)((app_seq_num >> 24) & 0xFF));

    // Increment for the next transmissions
    app_seq_num++;*/
}
...
void app_main(void)
{
	...
	ESP_LOGW(TAG, "TX Power set to minimum (-12 dBm) to simulate distance");
    ...
    xTaskCreate(mesh_stress_test_task, "mesh_stress_task", 4096, (void *)&root_models[1], 5, NULL);
}
