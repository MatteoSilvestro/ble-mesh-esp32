            ...
            if (param->status_cb.sensor_status.marshalled_sensor_data->len) {
                ...
                for (; length < param->status_cb.sensor_status.marshalled_sensor_data->len; ) {
                    ...
                    if (data_len != ESP_BLE_MESH_SENSOR_DATA_ZERO_LEN) {
                        if (prop_id == 0x0056 && data_len == 4) {
                            uint8_t *payload = data + mpid_len;
                            uint32_t rx_seq = (payload[1]) | (payload[2] << 8) | (payload[3] << 16) | (payload[4] << 24);
                            
                            total_received++;

                            if (rx_seq > expected_seq) {
                                uint32_t lost_in_transit = rx_seq - expected_seq;
                                total_lost += lost_in_transit;
                                ESP_LOGW(TAG, "COLLISION DETECTED: Lost %" PRIu32 " packets", lost_in_transit);
                            } else if (rx_seq < expected_seq && rx_seq == 0) {
                                ESP_LOGW(TAG, "Server rebooted. Resetting PDR counters");
                                total_lost = 0;
                                total_received = 1;
                            }
                            ...
                            float pdr = ((float)total_received / (total_received + total_lost)) * 100.0;
                            ESP_LOGI(TAG, "PDR: %.2f%% | Received: %" PRIu32 " | Lost: %" PRIu32, pdr, total_received, total_lost);
                        }
